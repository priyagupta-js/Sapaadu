...
function Homepage() {
  ...
  const handleCafeClick = (cafeId) => {
    navigate(`/menu/${cafeId}`); // Programmatically navigate to the '/menu' page
  };
  return (
    <>
      <Navbar />
      <div className="cafes-list">
        ...
        <div className="cafes-cont-wrapper">
          ...
          { cafes.length >0 ? (
            ...
            <div key={index} className="cafe1"
            // onClick={() =>handleCafeClick(cafe.id)}
            >
            <div className="cafe1-img" src={cafe.image} alt={cafe.name} />
            <div className="cafe1-det">
            <h2>{cafe.name}</h2>
            {/* <p>{ cafe.categories || "No Categories"}</p> */}
            </div>
            </div>
            ))
          ):
